		...
		<div class="cart_items">
			...
			<div class="cart_item" data-id="<%= currentItem.getMenuId() %>">
				<div class="item_details">
					...
					<p>
						&#8377;
						<%= currentItem.getPrice() %></p>
					<p
						id="total_<%= currentItem.getItemName().replaceAll("\\s+", "_") %>">
						Total: &#8377;
				...
				<div class="item_img">
					...
					<div class="inc_dec">
						<div class="counter-controls">
							<button class="action_btn" type="button"
								onclick="updateCounter(this, -1,<%= currentItem.getMenuId() %>,true)"
								data-item-name="<%= currentItem.getItemName() %>"
								data-item-price="<%= currentItem.getPrice() %>">-</button>
					...
					<i class="fa-solid fa-trash delete"
						onclick="deleteItem(<%= currentItem.getMenuId() %>, '<%= currentItem.getItemName() %>')"
						title="remove"></i>
				</div>
			</div>
			<%} %>
		...
		<div class="payment_section">
			<div class="amount">
				<table style="border-spacing: 0 10px; border-collapse: separate;">
					...
					<% CartItem currentItem = cartItem.getValue(); %>
					<tr
						id="item_<%= currentItem.getItemName().replaceAll("\\s+", "_") %>">
			...
			<div class="choosePayment">
				<label for="payment_method">Select Payment Method:</label> <select
					id="payment_method">
					<option value="UPI" selected>UPI</option>
					<option value="Cash On Delivery">Cash on Delivery</option>
				</select>
			</div>
			<button class="pay-btn" onclick="handleOrder()">Confirm
